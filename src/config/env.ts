...
// Read from process.env first (development), then from app.json extra (production builds)
const getEnvVar = (key: string): string => {
  return process.env[key] || Constants.expoConfig?.extra?.[key] || '';
};

const rawApiUrl = getEnvVar('EXPO_PUBLIC_API_URL').replace(/\/$/, '');
...
const rawSocketUrl = getEnvVar('EXPO_PUBLIC_SOCKET_URL').replace(/\/$/, '');

export const API_BASE_URL = normalizedApiUrl;
export const API_TIMEOUT = Number(getEnvVar('EXPO_PUBLIC_API_TIMEOUT') || 15000);
